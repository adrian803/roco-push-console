...
控制台粘贴部署在 Dashboard → Variables 中添加；Wrangler 部署在 `wrangler.toml` 的 `[vars]` 中设置。
...
CF Workers 版将核心推送逻辑打包为单文件 [cf-workers/_worker.js](cf-workers/_worker.js)，使用 Cloudflare Cron Triggers 定时执行。支持三种部署方式：控制台粘贴 `_worker.js`、Workers Builds 从 GitHub/GitLab 项目部署，或本地 Wrangler 部署。三种方式使用同一份源码，`_worker.js` 是从 `src/` 构建出的产物。
...
**如何更新到最新版本？** 控制台粘贴部署：复制仓库或 GitHub Release 附件中的新版 [_worker.js](cf-workers/_worker.js) 全量内容，替换 Worker 编辑器中的代码并保存部署。Wrangler 项目部署：`git pull && npm ci && npm run deploy`。
