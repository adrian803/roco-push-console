...
**方式 D：一键脚本部署（PowerShell + API Token）**

这是对方式 C 的封装，也是本仓库实际验证过的 Wrangler 项目部署路径。脚本会自动进入 `cf-workers`，依次执行 `npm ci`、Worker 单测、TypeScript 检查、`_worker.js` 同步检查、可选写入 secrets、`wrangler deploy`，最后访问根路径 `/` 做健康检查。

准备项：

- 本机已安装 Node.js 和 PowerShell
...
.\scripts\deploy-cf-worker.ps1 -ConfigureSecrets
Remove-Item Env:CLOUDFLARE_API_TOKEN
```

`-ConfigureSecrets` 会依次提示输入 `ROCOM_API_KEY`、`SERVERCHAN_SENDKEY` 和可选 `TRIGGER_TOKEN`，并通过 `wrangler secret put` 写入 Cloudflare，不会写入仓库文件。也可以完全交互式运行，脚本会隐藏提示输入 `CLOUDFLARE_API_TOKEN`：
...
```powershell
.\scripts\deploy-cf-worker.ps1 -DryRun
```
...
- Windows / PowerShell 用户：运行 [scripts/deploy-cf-worker.ps1](scripts/deploy-cf-worker.ps1) 一键安装依赖、检查、配置 secrets、部署并验证根路径。
...
```powershell
# 首次部署：提示输入 Cloudflare API Token、ROCOM_API_KEY、SERVERCHAN_SENDKEY、TRIGGER_TOKEN
.\scripts\deploy-cf-worker.ps1 -ConfigureSecrets
...
**如何更新到最新版本？** 控制台粘贴部署：复制仓库或 GitHub Release 附件中的新版 [_worker.js](cf-workers/_worker.js) 全量内容，替换 Worker 编辑器中的代码并保存部署。Workers Builds：推送到生产分支后自动部署。一键脚本部署：`git pull` 后运行 `.\scripts\deploy-cf-worker.ps1`。手动 Wrangler 项目部署：`git pull && cd cf-workers && npm ci && npm run deploy`。
